...
namespace PRADA_Vayne.MyLogic.Others
{
    public static partial class Events
    {
        ...
        {
            if (Program.DrawingsMenu["drawenemywaypoints"].GetValue<MenuBool>().Enabled)
                foreach (var e in GameObjects.EnemyHeroes.Where(en => en.IsVisible && !en.IsDead && en.Distance(Heroes.Player) < 2500))
                {
