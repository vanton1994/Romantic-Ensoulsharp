    ...
    class MasterYi
    {
        ...
        public static void slayMaderDuker(AIBaseClient target)
        {
            try
            {
                ...
                if (MasterSharp.mainMenu.Item("useQ").GetValue<MenuBool>() && (Orbwalker.CanMove() || Q.IsKillable(target)))
                    useQSmart(target);
                if (MasterSharp.mainMenu.Item("useE").GetValue<MenuBool>())
                    useESmart(target);
                if (MasterSharp.mainMenu.Item("useR").GetValue<MenuBool>())
                    useRSmart(target);
            }
            catch (Exception ex)
            {
        ...
        {
            if (useQ != 0 && Q.IsReady() && jumpEnesAround() != 0 && MasterSharp.mainMenu.Item("smartQDogue").GetValue<MenuBool>())
            {
                //Game.Print("Cast Q");
                if (delay != 0)
                    Activator.GetObject(delay, useQonBest);
                else
                    useQonBest();
            }
            else if (useW != 0 && W.IsReady() && MasterSharp.mainMenu.Item("smartW").GetValue<MenuBool>())
            {
            ...
            try
            {
                ...
                if (selectedTarget != null)
                {
                    ...
                    if (selectedTarget.Distance(player) < 600)
                    {
                        ...
                        Q.Cast(selectedTarget, MasterSharp.mainMenu.Item("packets").GetValue<MenuBool>());
                        return;
                    }
                    ...
                    if (bestOther != null)
                    {
                        Q.Cast(bestOther, MasterSharp.mainMenu.Item("packets").GetValue<MenuBool>());
                    }
                }
                else
                {
    ...
    internal class MasterSharp
    {
        ...
        public MasterSharp()
        {
            ...
            TargetedSkills.setUpSkills();
                ...
                mainMenu = new Menu("MasterYi", "[Heler]MasterYi", true);

            //TS
            //Combo
            var Combo = new Menu("combo", "Combo Sharp");
                Combo.Add(new MenuBool("comboWreset", "AA reset W"));
                Combo.Add(new MenuBool("useQ", "Use Q to gap"));
            ...
            var Extra = new Menu( "extra", "Extra Sharp");
                Extra.Add(new MenuBool("packets", "Use Packet cast", false));

                Extra.Add(new MenuBool("aShots", "Anti Skillshots"));
            mainMenu.Add(Extra);
                //SmartW
            var SmartW = new MenuBool("SmartW", "Smart W");
                SmartW = new MenuBool("smartW", "Smart W if cantQ");
                SmartW = new MenuBool("smartQDogue", "Q use evade");
                SmartW = new MenuSlider("useWatHP", "use W below HP", 100, 0, 100);
                SmartW = new MenuBool("wqOnDead", "W or Q if will kill", false);
            mainMenu.Add(SmartW);

            //Debug
                var draw = new MenuBool("draw", "Drawing");
                draw = new MenuBool("drawCir", "Draw circles");
            mainMenu.Add(draw);
                mainMenu.Attach();
        ...
        private static void afterAttack(Object sender, Orbwalker args)
        {
            if(args.Sender == null)
            {
                return;
            }
            if (args.Sender.IsMe && args.GetType == Orbwalker)
            {
                if (W.IsReady() && mainMenu("comboWreset").GetValue<MenuBool>() && mainMenu.Item("useWatHP").GetValue<MenuSlider>().Value >= player.HealthPercent && args is AIHeroClient && Orbwalker.ActiveMode == OrbwalkerMode.Combo)
                {
                    W.Cast();
                    Activator.Add(100, delegate { Orbwalker.ResetAutoAttackTimer(); });
        ...
        private static void OnGameUpdate(EventArgs args)
        {
            ...
            if (mainMenu.Item("debugOn").GetValue<MenuKeyBind>().Active) //fullDMG
            {
        ...
        private static void onDraw(EventArgs args)
        {
            ...
            if (!mainMenu.Item("drawCir").GetValue<MenuBool>())
                return;
